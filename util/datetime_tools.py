...
def client2str(client_time, fm="%Y%m%d", utc=True):
    dt = client2datetime(client_time, utc) 
    if not dt:
        return
    return datetime2str(dt, fm)


def client2datetime(client_time, utc=True):
    # format: "2019-08-14T06:26:19.683+0530"
    try:
...
def client2timestamp(client_time, ms=False, utc=True):
    dt = client2datetime(client_time, utc)
    if not dt:
        return
    return datetime2timestamp(dt, ms)


def client2timestamp_v2(client_time, ms=False):
    pattern = re.compile('^(?P<datetime>[0-9]{2,4}\-[0-9]{1,2}\-[0-9]{1,2}T[0-9]{1,2}:[0-9]{1,2}:[0-9\.]{1,6})'
                         '(?P<timezone>[\+\-][0-9]{4})?$')
    result = pattern.match(client_time)
    if not result:
        return -1
    time_info = result.groupdict()

    # 时区的处理
    datetime_parts = time_info.get('datetime').split('.')
    if len(datetime_parts) == 2:
        msec = int(datetime_parts[1] or 0)
    else:
        msec = 0
    datetime = datetime_parts[0]

    # 防止有些数据不带时区
    timezone = int(time_info.get('timezone') or "+0000")
    delta_seconds = (int(abs(timezone) / 100) * 60 + abs(timezone) % 100) * 60
    if timezone >= 0:
        delta_seconds = -delta_seconds

    time_data = time.strptime(datetime, "%Y-%m-%dT%H:%M:%S")
    timestamp = time.mktime(time_data) + delta_seconds - time.timezone
...
def get_date_ago(start_date, duration=1, fm="%Y%m%d"):
    dt_start = str2datetime(start_date)
